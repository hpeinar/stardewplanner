...
function parseSaveFile(filePath) {
    ...
    return new Promise(function (resolve, reject) {
        fs.readFile(filePath, (err, save) => {
            ...
            fs.rename(filePath, __dirname +'/../saves/'+ xmlFileName, function (err) {
                if (err) {
                    console.error('Failed to move save xml', err);
                }
            });

            let xmlDoc = libxmljs.parseXml(save.toString('utf8'));

            let playerName, farmName;
            let farmLocation, farmItems, buildings, resourceClumps, terrainFeatures = [];
            ...
            // save file
            fs.writeFile(__dirname +'/../farms/'+ saveFileName, saveData, function (err) {
                if (err) {
                    reject(err);
                    return;
                }
