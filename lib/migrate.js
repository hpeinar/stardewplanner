...
db.setup(console).then(function () {
    r.connect(config.rethinkdb).then(function (conn) {
        ...
        fs.readdir('./farms/', function (err, files) {
            ...
            async.mapLimit(files, 10, function (longFileName, cb) {
                (new Promise(function (resolve, reject) {
                    return migrateFarm(longFileName, conn, resolve, reject);
                    // let oldId = longFileName.split('.')[0];
                    //
                    // r.table('farms').getAll(oldId, {index: 'oldId'}).run(conn).then(farms => farms.toArray()).then(function (oldFarms) {
                    ...
                    console.log('Done with index', files.indexOf(longFileName));
                    cb(null);
                }).catch(function (err) {
                    cb(err);
                });
